...
module cv2
    ...
    function cpp_KeyPoint_to_jl_KeyPoint(kp)
        kpr = KeyPoint(cpp_Point2f_to_jl_tuple(kp.pt), kp.size, 
    ...
    function imshow(winname::String, img::Image)
        jl_cpp_cv2.imshow(winname, cv2.jl_arr_to_cpp_mat(img))
    end
    function imread(filename::String, flags::Integer)
        cv_Mat_tmp = jl_cpp_cv2.imread(filename, flags)
    ...
    function CascadeClassifier_detectMultiScale(inp1, image::Image;
        ...
        print("a")
        print(jl_size_to_cpp_size(minSize), jl_size_to_cpp_size(maxSize))
